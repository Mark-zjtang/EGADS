...
def imagine_ahead(prev_state, prev_belief, policy, transition_model, planning_horizon=12):
  ...
  for t in range(T - 1):
    ...
    hidden = transition_model.act_fn(transition_model.fc_embed_state_action3(transition_model.act_fn(transition_model.fc_embed_state_action2(transition_model.act_fn(transition_model.fc_embed_state_action1(torch.cat([_state, actions], dim=1)))))))
    beliefs[t + 1] = transition_model.rnn(hidden, beliefs[t])
    # Compute state prior by applying transition dynamics
    hidden = transition_model.act_fn(transition_model.fc_state_prior2(transition_model.act_fn(transition_model.fc_state_prior1(transition_model.act_fn(transition_model.fc_embed_belief_prior(beliefs[t + 1]))))))
    prior_means[t + 1], _prior_std_dev = torch.chunk(transition_model.fc_state_prior3(hidden), 2, dim=1)
...
def imagine_ahead_flow_batch(prev_state, prev_belief, policy, transition_model, planning_horizon, fix_prior, freeze_context, traj_batch):
  '''
  imagine_ahead is the function to draw the imaginary tracjectory using the dynamics model, actor, critic.
  Input: current state (posterior), current belief (hidden), policy, transition_model  # torch.Size([50, 30]) torch.Size([50, 200]) 
  Output: generated trajectory of features includes beliefs, prior_states, prior_means, prior_std_devs
  ...
  T = planning_horizon
  beliefs, prior_states, prior_means, prior_std_devs = [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T
  policy_ent = [torch.empty(0)] * T
  prev_belief = prev_belief[None, :].repeat(traj_batch, 1, 1).view(-1, prev_belief.shape[-1])
  prev_state = prev_state[None, :].repeat(traj_batch, 1, 1).view(-1, prev_state.shape[-1])
  ...
  for t in range(T - 1):
    ...
    # actions = policy.get_action(beliefs[t].detach(),_state.detach())
    actions, ent = policy.get_action(beliefs[t].detach(),_state.detach(), get_entropy=True)
    policy_ent[t + 1] = ent 
    # Compute belief (deterministic hidden state)
    ...
    # Choice A. context = MLP embedding
      # h = transition_model.act_fn(transition_model.fc_embed_belief_flow_prior_context1(beliefs[t + 1]))
      # cont = transition_model.fc_embed_belief_flow_prior_context2(h)
    # Choice B. context = belief
    cont = beliefs[t + 1]

    # Prior Base Samples
    prior_states_ = prior_means[t + 1] + prior_std_devs[t + 1] * torch.randn_like(prior_means[t + 1])
    prior_states[t + 1] = transition_model._transfer_from_base(prior_states_, cont, is_prior=True, freeze_context=freeze_context)  # transfered samples
  
  b = torch.stack(beliefs[1:], dim=0).view(T-1, traj_batch, -1, beliefs[0].shape[-1])
  b = torch.transpose(b, 0, 1)

  ps = torch.stack(prior_states[1:], dim=0).view(T-1, traj_batch, -1, prior_states[0].shape[-1])
  ps = torch.transpose(ps, 0, 1)

  pm = torch.stack(prior_means[1:], dim=0).view(T-1, traj_batch, -1, prior_means[-1].shape[-1])
  pm = torch.transpose(pm, 0, 1)

  psd = torch.stack(prior_std_devs[1:], dim=0).view(T-1, traj_batch, -1, prior_std_devs[-1].shape[-1])
  psd = torch.transpose(psd, 0, 1)

  pe = torch.stack(policy_ent[1:], dim=0).view(T-1, traj_batch, -1)
  pe = torch.transpose(pe, 0, 1)


  # Return new hidden states
  imagined_traj = [b, ps, pm, psd, pe]
  return imagined_traj

def imagine_ahead_flow(prev_state, prev_belief, policy, transition_model, planning_horizon, fix_prior, freeze_context):
  '''
  imagine_ahead is the function to draw the imaginary tracjectory using the dynamics model, actor, critic.
  Input: current state (posterior), current belief (hidden), policy, transition_model  # torch.Size([50, 30]) torch.Size([50, 200]) 
  Output: generated trajectory of features includes beliefs, prior_states, prior_means, prior_std_devs
  ...
  # Return new hidden states
  imagined_traj = [torch.stack(beliefs[1:], dim=0), torch.stack(prior_states[1:], dim=0), torch.stack(prior_means[1:], dim=0), torch.stack(prior_std_devs[1:], dim=0), torch.stack(policy_ent[1:], dim=0)]
  return imagined_traj
