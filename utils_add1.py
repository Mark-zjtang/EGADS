...
def imagine_ahead(prev_state, prev_belief, policy, transition_model, planning_horizon=12):
  ...
  for t in range(T - 1):
    ...
    hidden = transition_model.act_fn(transition_model.fc_embed_state_action2(transition_model.act_fn(transition_model.fc_embed_state_action1(torch.cat([_state, actions], dim=1)))))
    beliefs[t + 1] = transition_model.rnn(hidden, beliefs[t])
    # Compute state prior by applying transition dynamics
    hidden = transition_model.act_fn(transition_model.fc_state_prior1(transition_model.act_fn(transition_model.fc_embed_belief_prior(beliefs[t + 1]))))
    prior_means[t + 1], _prior_std_dev = torch.chunk(transition_model.fc_state_prior2(hidden), 2, dim=1)
